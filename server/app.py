...
# IMPORTANT: Marshmallow must be instantiated AFTER db.init_app(app)
ma = Marshmallow(app)

# ---------------------------------------------------------
# Marshmallow Schema with HATEOAS hyperlinks
# ---------------------------------------------------------
class NewsletterSchema(ma.SQLAlchemySchema):
    ...
    # Include only selected fields in list view (HATEOAS style)
    title = ma.auto_field()
    published_at = ma.auto_field()

    url = ma.Hyperlinks(
        {
            "self": ma.URLFor("newsletterbyid", values={"id": "<id>"}),
